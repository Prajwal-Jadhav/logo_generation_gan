...
PATH = "C:\\Users\\prajwalj\\Desktop\\pytoch_cgan\\"
TRAINING = True
VERBOSE = True
...
def one_hot_encode(data, folder):
    ...
    one_hot_encoding.to_csv("labels.csv", index = False, header = False)
...
for folder in folders:
    ...
    for img in colors.index:
        ...
        if VERBOSE: 
                if TRAINING:
                    sys.stdout.write("Progress: {:.2%}\r".format(cnt/51744))
                    sys.stdout.flush()
                else:
    ...
    for img in colors.index:
        ...
        if len(color_word_sorted) > 2:
            ...
            word_most.append(color_word_sorted[0])
        ...
        if len(color_word_sorted) > 1:
            ...
            compact_middle.append(color_word_compact_sorted[0])

        if len(color_word_sorted) > 2:
            compact_most.append(color_word_compact_sorted[2])  
        else:
            compact_most.append(color_word_compact_sorted[0])       
